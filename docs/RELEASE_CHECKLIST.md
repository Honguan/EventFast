...
最新本機候選：`artifacts/release-20260823-14/win-x64/EventFast.exe`

SHA256：`a3af2b0e17a8a2ff01f8ca0a50161977718857aee278cdfdd3dc9d79890db37d`
...
- [x] 三次全新 bundle extraction 冷啟動 816／687／700 ms，中位數 700 ms（門檻 < 1 秒）
- [x] 三次執行中正式候選沒有 TCP／UDP endpoint（門檻皆為 0）
- [x] 三次正常關閉後，正式候選程序皆於 2 秒內完全退出
- [x] Published EXE self-test 與 UI startup smoke 成功
- [x] 第一批 callback 先回傳 Native System 欄位，最終欄位與事件 XML 逐欄一致，查詢工作不在 UI thread
- [x] 10k／100k／500k／1M 群組及 XLSX benchmark 完成
- [x] 產生的 XLSX 已由本機 Microsoft Excel 開啟並確認兩張工作表
- [x] 500 次 native query/message 循環（handle -3、private memory +10.9 MB，門檻 +10／+32 MB）
- [x] SHA256 產生完成
...
- [x] 62,031 筆公開真實 EVTX 完整讀取（第一批 3.5 ms、44.9 MB managed memory）
...
- [x] UI 驗證（命令列自動查詢、雙 Channel 32 筆／98 ms；候選檔 Enter、雙擊、Esc、Message/XML）
